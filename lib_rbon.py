...
def flip(f):  #1 (line num in coconut source)
    return (lambda x, y: (f)(*(y, x)))  #1 (line num in coconut source)

@_coconut_tco  #3 (line num in coconut source)
def uncurry(f):  #3 (line num in coconut source)
    @_coconut_tco  #3 (line num in coconut source)
    @_coconut_mark_as_match  #3 (line num in coconut source)
    def g(*_coconut_match_args, **_coconut_match_kwargs):  #3 (line num in coconut source)
        _coconut_match_check_0 = False  #3 (line num in coconut source)
        _coconut_match_set_name_f = _coconut_sentinel  #3 (line num in coconut source)
        _coconut_match_set_name_x = _coconut_sentinel  #3 (line num in coconut source)
        _coconut_match_set_name_y = _coconut_sentinel  #3 (line num in coconut source)
        _coconut_FunctionMatchError = _coconut_get_function_match_error()  #3 (line num in coconut source)
        if (_coconut.len(_coconut_match_args) == 2) and ("f" not in _coconut_match_kwargs) and (_coconut.isinstance(_coconut_match_args[1], _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_args[1]) == 2):  #3 (line num in coconut source)
            _coconut_match_temp_0 = _coconut_match_args[0] if _coconut.len(_coconut_match_args) > 0 else _coconut_match_kwargs.pop("f")  #3 (line num in coconut source)
            _coconut_match_set_name_x = _coconut_match_args[1][0]  #3 (line num in coconut source)
            _coconut_match_set_name_y = _coconut_match_args[1][1]  #3 (line num in coconut source)
            if not _coconut_match_kwargs:  #3 (line num in coconut source)
                _coconut_match_set_name_f = _coconut_match_temp_0  #3 (line num in coconut source)
                _coconut_match_check_0 = True  #3 (line num in coconut source)
        if _coconut_match_check_0:  #3 (line num in coconut source)
            if _coconut_match_set_name_f is not _coconut_sentinel:  #3 (line num in coconut source)
                f = _coconut_match_temp_0  #3 (line num in coconut source)
            if _coconut_match_set_name_x is not _coconut_sentinel:  #3 (line num in coconut source)
                x = _coconut_match_args[1][0]  #3 (line num in coconut source)
            if _coconut_match_set_name_y is not _coconut_sentinel:  #3 (line num in coconut source)
                y = _coconut_match_args[1][1]  #3 (line num in coconut source)
        if not _coconut_match_check_0:  #3 (line num in coconut source)
            raise _coconut_FunctionMatchError('def g(f, [x,y]) = (x, y) |*> f', _coconut_match_args)  #3 (line num in coconut source)

        return _coconut_tail_call((f), *(x, y))  #4 (line num in coconut source)

    return _coconut_tail_call(_coconut.functools.partial(_coconut.functools.partial, g), f)  #6 (line num in coconut source)
split = (flip)(str.split)  #6 (line num in coconut source)
curry = _coconut.functools.partial  #7 (line num in coconut source)

def exec_time(f, *args):  #9 (line num in coconut source)
    start_time = time.perf_counter()  #10 (line num in coconut source)
    output = f(*args)  #11 (line num in coconut source)
    end_time = time.perf_counter()  #12 (line num in coconut source)
    (print)("finished in: " + ((str)(end_time - start_time)))  #13 (line num in coconut source)
    return (output)  #14 (line num in coconut source)
