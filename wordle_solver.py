...
def step(command):  #6 (line num in coconut source)
# parse the command into info
    pairs = (zip)(*(split)(" = ", command))  #8 (line num in coconut source)
    greens = []  #9 (line num in coconut source)
    yellows = []  #10 (line num in coconut source)
    bads = []  #11 (line num in coconut source)
    unknown = [0, 1, 2, 3, 4]  #12 (line num in coconut source)
    index = 0  #13 (line num in coconut source)
    for i in pairs:  #14 (line num in coconut source)
        if i[1] == 'g':  #15 (line num in coconut source)
            greens.append((i[0], index))  #16 (line num in coconut source)
            if index in unknown:  #17 (line num in coconut source)
                unknown.remove(index)  #18 (line num in coconut source)
        elif i[1] == 'y':  #19 (line num in coconut source)
            yellows.append((i[0], index))  #20 (line num in coconut source)
        elif i[1] == 'b':  #21 (line num in coconut source)
            bads.append((i[0]))  #22 (line num in coconut source)
        index += 1  #23 (line num in coconut source)

    alphabet = 'abcdefghijklmnopqrstuvwxyz'  #25 (line num in coconut source)

# remove bad letters from the pool
    chars = (tuple)((filter)(lambda y: y not in bads, alphabet))  #28 (line num in coconut source)

    (print)("generating words sans bad letters...")  #30 (line num in coconut source)
    possible_words = (tuple)(product(chars, repeat=5))  #31 (line num in coconut source)
    (print)('total words: ' + ((str)((len)(possible_words))) + '\n')  #32 (line num in coconut source)

    possible_words = (list)((filter)(_coconut.functools.partial(check_word, greens, yellows, unknown), possible_words))  # this is a list not a tuple to match the stable test  #34 (line num in coconut source)

    print("TOTAL WORDS: " + str(len(possible_words)))  #38 (line num in coconut source)

    return (possible_words)  #40 (line num in coconut source)

def fits_green(word, green):  #42 (line num in coconut source)
    return (word[green[1]] == green[0])  #42 (line num in coconut source)

def fits_all_greens(greens, word):  #44 (line num in coconut source)
    for green in greens:  #45 (line num in coconut source)
        if word[green[1]] != green[0]:  #46 (line num in coconut source)
            return (False)  #47 (line num in coconut source)
    return (True)  #48 (line num in coconut source)

def fits_yellow(word, unknown, yellow):  #50 (line num in coconut source)
    if word[yellow[1]] != yellow[0]:  #51 (line num in coconut source)
        res = False  #52 (line num in coconut source)
        for u in unknown:  #53 (line num in coconut source)
            if word[u] == yellow[0]:  #54 (line num in coconut source)
                return (True)  #55 (line num in coconut source)
    return (False)  #56 (line num in coconut source)

def fits_all_yellows(yellows, unknown, word):  #58 (line num in coconut source)
    for yellow in yellows:  #59 (line num in coconut source)
        if not fits_yellow(word, unknown, yellow):  #60 (line num in coconut source)
            return (False)  #61 (line num in coconut source)
    return (True)  #62 (line num in coconut source)

def check_word(greens, yellows, unknown, word):  #64 (line num in coconut source)
    if (fits_all_greens)(greens, word):  #65 (line num in coconut source)
        if (fits_all_yellows)(yellows, unknown, word):  #66 (line num in coconut source)
            if ''.join(word) in words.all_words:  #67 (line num in coconut source)
                return (True)  #68 (line num in coconut source)
    return (False)  #69 (line num in coconut source)

@_coconut_tco  #71 (line num in coconut source)
def count_vowels(word):  #71 (line num in coconut source)
    return _coconut_tail_call((len), (tuple)((filter)(lambda x: x in 'aeiouy', word)))  #71 (line num in coconut source)

def reveal_4_vowel_words(word):  #73 (line num in coconut source)
    if ((count_vowels)(word)) == 4:  #74 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #75 (line num in coconut source)
            print(word)  #76 (line num in coconut source)

def reveal_5_vowel_words(word):  #78 (line num in coconut source)
    if ((count_vowels)(word)) == 5:  #79 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #80 (line num in coconut source)
            print(word)  #81 (line num in coconut source)

@_coconut_tco  #83 (line num in coconut source)
def fancy_format(iterable):  #83 (line num in coconut source)
    return _coconut_tail_call('\n'.join, tuple(map(''.join, iterable)))  #83 (line num in coconut source)

if __name__ == '__main__':  #85 (line num in coconut source)
    (print)('')  #86 (line num in coconut source)
# words.all_words |> fmap $ reveal_4_vowel_words
    (print)((fancy_format)((step)("audio = ybgby")))  #88 (line num in coconut source)
