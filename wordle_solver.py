...
from itertools import product  #5 (line num in coconut source)
import words  #6 (line num in coconut source)
from lib_rbon import flip  #7 (line num in coconut source)
from lib_rbon import uncurry  #7 (line num in coconut source)
from lib_rbon import split  #7 (line num in coconut source)
import time  #8 (line num in coconut source)

@_coconut_tco  #10 (line num in coconut source)
def step(command):  #10 (line num in coconut source)

    @_coconut_tco  #12 (line num in coconut source)
    def filter_at_position(xs, y, pos):  #12 (line num in coconut source)
        return _coconut_tail_call((tuple), (filter)(lambda x: x[pos] == y, xs))  #12 (line num in coconut source)

    @_coconut_tco  #14 (line num in coconut source)
    def filter_not_at_position(xs, y, pos):  #14 (line num in coconut source)
        return _coconut_tail_call((tuple), (filter)(lambda x: x[pos] != y, xs))  #15 (line num in coconut source)

    @_coconut_tco  #17 (line num in coconut source)
    def filter_contains(xs, y):  #17 (line num in coconut source)
        return _coconut_tail_call((tuple), (filter)(lambda x: y in x, xs))  #17 (line num in coconut source)

    @_coconut_tco  #19 (line num in coconut source)
    def filter_at_any_of(xs, y, ps):  #19 (line num in coconut source)
        return _coconut_tail_call((tuple), (filter)(_coconut.functools.partial(at_any_of, y, ps), xs))  #20 (line num in coconut source)

    def at_any_of(y, ps, x):  #22 (line num in coconut source)
        for p in ps:  #23 (line num in coconut source)
            if x[p] == y:  #24 (line num in coconut source)
                return (True)  #25 (line num in coconut source)
        return (False)  #26 (line num in coconut source)

    def make_info(pairs):  #28 (line num in coconut source)
        greens = []  #29 (line num in coconut source)
        yellows = []  #30 (line num in coconut source)
        bads = []  #31 (line num in coconut source)
        unknown = [0, 1, 2, 3, 4]  #32 (line num in coconut source)
        index = 0  #33 (line num in coconut source)
        for i in pairs:  #34 (line num in coconut source)
            if i[1] == 'g':  #35 (line num in coconut source)
                greens.append((i[0], index))  #36 (line num in coconut source)
                if index in unknown:  #37 (line num in coconut source)
                    unknown.remove(index)  #38 (line num in coconut source)
            elif i[1] == 'y':  #39 (line num in coconut source)
                yellows.append((i[0], index))  #40 (line num in coconut source)
            elif i[1] == 'b':  #41 (line num in coconut source)
                bads.append((i[0]))  #42 (line num in coconut source)
            index += 1  #43 (line num in coconut source)
        return ({'greens': greens, 'yellows': yellows, 'bads': bads, 'unknown': unknown})  #44 (line num in coconut source)

    def generate_words(chars):  #50 (line num in coconut source)
        (print)("generating words sans bad letters...")  #51 (line num in coconut source)
        output = (tuple)(product(chars, repeat=5))  #52 (line num in coconut source)
        (print)('total words: ' + ((str)((len)(output))))  #53 (line num in coconut source)
        return (output)  #54 (line num in coconut source)

    def filter_with_greens(greens, words):  #56 (line num in coconut source)
        (print)("filtering greens...")  #57 (line num in coconut source)
        output = words  #58 (line num in coconut source)
        for i in greens:  #59 (line num in coconut source)
            output = filter_at_position(output, i[0], i[1])  #60 (line num in coconut source)
        output = (tuple)(output)  #61 (line num in coconut source)
        (print)('green guesses: ' + ((str)((len)(output))))  #62 (line num in coconut source)
        return (output)  #63 (line num in coconut source)

    def filter_with_yellows(yellows, unknown, words):  #65 (line num in coconut source)
        (print)("filtering yellows...")  #66 (line num in coconut source)
        output = words  #67 (line num in coconut source)
        for i in yellows:  #68 (line num in coconut source)
            output = filter_not_at_position(output, i[0], i[1])  #69 (line num in coconut source)
            output = filter_at_any_of(output, i[0], unknown)  #70 (line num in coconut source)
        output = (tuple)(output)  #71 (line num in coconut source)
        (print)('yellow guesses: ' + ((str)((len)(output))))  #72 (line num in coconut source)
        return (output)  #73 (line num in coconut source)

    def filter_actual_words(guesses):  #75 (line num in coconut source)
        (print)("filtering actual words...")  #76 (line num in coconut source)
        output = [x for x in guesses if ''.join(x) in words.all_words]  #77 (line num in coconut source)
        (print)('possible solutions: ' + ((str)((len)((tuple)(output)))))  #78 (line num in coconut source)
        return (output)  #79 (line num in coconut source)

    @_coconut_tco  #81 (line num in coconut source)
    def remove_matches(xs, ys):  #81 (line num in coconut source)
        return _coconut_tail_call((tuple), (filter)(lambda y: y not in xs, ys))  #81 (line num in coconut source)

# parse the command into a dict of info
    info = (make_info)((zip)(*(split)(" = ", command)))  #84 (line num in coconut source)

    return _coconut_tail_call((exec_time), filter_actual_words, (exec_time)(_coconut.functools.partial(filter_with_yellows, info['yellows'], info['unknown']), (exec_time)(_coconut.functools.partial(filter_with_greens, info['greens']), (exec_time)(generate_words, (remove_matches)(info["bads"], 'abcdefghijklmnopqrstuvwxyz')))))  #86 (line num in coconut source)

def exec_time(f, *args):  #93 (line num in coconut source)
    start_time = time.perf_counter()  #94 (line num in coconut source)
    output = f(*args)  #95 (line num in coconut source)
    end_time = time.perf_counter()  #96 (line num in coconut source)
    (print)("finished in: " + ((str)(end_time - start_time)) + '\n')  #97 (line num in coconut source)
    return (output)  #98 (line num in coconut source)

@_coconut_tco  #100 (line num in coconut source)
def count_vowels(word):  #100 (line num in coconut source)
    return _coconut_tail_call((len), (tuple)((filter)(lambda x: x in 'aeiouy', word)))  #100 (line num in coconut source)

def reveal_4_vowel_words(word):  #102 (line num in coconut source)
    if ((count_vowels)(word)) == 4:  #103 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #104 (line num in coconut source)
            print(word)  #105 (line num in coconut source)

def reveal_5_vowel_words(word):  #107 (line num in coconut source)
    if ((count_vowels)(word)) == 5:  #108 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #109 (line num in coconut source)
            print(word)  #110 (line num in coconut source)

@_coconut_tco  #112 (line num in coconut source)
def fancy_print(iterable):  #112 (line num in coconut source)
    return _coconut_tail_call('\n'.join, tuple(map(''.join, iterable)))  #112 (line num in coconut source)

if __name__ == '__main__':  #114 (line num in coconut source)
    (print)('')  #115 (line num in coconut source)
# words.all_words |> fmap $ reveal_4_vowel_words
    (print)((fancy_print)((step)("apdlp = ggbgy")))  #117 (line num in coconut source)
