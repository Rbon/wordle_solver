...
def fancy_print(iterable):  #10 (line num in coconut source)
    return _coconut_tail_call('\n'.join, tuple(map(''.join, iterable)))  #10 (line num in coconut source)

@_coconut_tco  #12 (line num in coconut source)
def parse(command):  #12 (line num in coconut source)
    return _coconut_tail_call((make_info), (zip)(*(split)(" = ", command)))  #12 (line num in coconut source)

def make_info(pairs):  #14 (line num in coconut source)
    ...
    return ({'greens': greens, 'yellows': yellows, 'bads': bads, 'unknown': unknown})  #30 (line num in coconut source)

@_coconut_tco  #36 (line num in coconut source)
def remove_matches(xs, ys):  #36 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda y: y not in xs, ys))  #36 (line num in coconut source)

def generate_words(chars):  #38 (line num in coconut source)
    (print)("generating words sans bad letters...")  #39 (line num in coconut source)
    output = (tuple)(product(chars, repeat=5))  #40 (line num in coconut source)
    (print)('total words: ' + ((str)((len)(output))))  #41 (line num in coconut source)
    return (output)  #42 (line num in coconut source)


@_coconut_tco  #45 (line num in coconut source)
def step(info):  #45 (line num in coconut source)
    return _coconut_tail_call((exec_time), filter_actual_words, (exec_time)(_coconut.functools.partial(filter_with_yellows, info['yellows'], info['unknown']), (exec_time)(_coconut.functools.partial(filter_with_greens, info['greens']), (exec_time)(generate_words, (remove_matches)(info["bads"], 'abcdefghijklmnopqrstuvwxyz')))))  #46 (line num in coconut source)

def filter_actual_words(guesses):  #53 (line num in coconut source)
    (print)("filtering actual words...")  #54 (line num in coconut source)
    output = [x for x in guesses if ''.join(x) in words.all_words]  #55 (line num in coconut source)
    (print)('possible solutions: ' + ((str)((len)((tuple)(output)))))  #56 (line num in coconut source)
    return (output)  #57 (line num in coconut source)

@_coconut_tco  #59 (line num in coconut source)
def filter_at_position(xs, y, pos):  #59 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda x: x[pos] == y, xs))  #59 (line num in coconut source)

@_coconut_tco  #61 (line num in coconut source)
def filter_not_at_position(xs, y, pos):  #61 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda x: x[pos] != y, xs))  #62 (line num in coconut source)

@_coconut_tco  #64 (line num in coconut source)
def filter_contains(xs, y):  #64 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda x: y in x, xs))  #64 (line num in coconut source)

def exec_time(f, *args):  #66 (line num in coconut source)
    start_time = time.perf_counter()  #67 (line num in coconut source)
    output = f(*args)  #68 (line num in coconut source)
    end_time = time.perf_counter()  #69 (line num in coconut source)
    (print)("finished in: " + ((str)(end_time - start_time)) + '\n')  #70 (line num in coconut source)
    return (output)  #71 (line num in coconut source)

def filter_with_greens(greens, words):  #73 (line num in coconut source)
    (print)("filtering greens...")  #74 (line num in coconut source)
    output = words  #75 (line num in coconut source)
    for i in greens:  #76 (line num in coconut source)
        output = filter_at_position(output, i[0], i[1])  #77 (line num in coconut source)
    output = (tuple)(output)  #78 (line num in coconut source)
    (print)('green guesses: ' + ((str)((len)(output))))  #79 (line num in coconut source)
    return (output)  #80 (line num in coconut source)

def filter_with_yellows(yellows, unknown, words):  #82 (line num in coconut source)
    (print)("filtering yellows...")  #83 (line num in coconut source)
    output = words  #84 (line num in coconut source)
    for i in yellows:  #85 (line num in coconut source)
        output = filter_not_at_position(output, i[0], i[1])  #86 (line num in coconut source)
        output = filter_at_any_of(output, i[0], unknown)  #87 (line num in coconut source)
    output = (tuple)(output)  #88 (line num in coconut source)
    (print)('yellow guesses: ' + ((str)((len)(output))))  #89 (line num in coconut source)
    return (output)  #90 (line num in coconut source)

@_coconut_tco  #92 (line num in coconut source)
def filter_at_any_of(xs, y, ps):  #92 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(_coconut.functools.partial(at_any_of, y, ps), xs))  #93 (line num in coconut source)

def at_any_of(y, ps, x):  #95 (line num in coconut source)
    for p in ps:  #96 (line num in coconut source)
        if x[p] == y:  #97 (line num in coconut source)
            return (True)  #98 (line num in coconut source)
    return (False)  #99 (line num in coconut source)

@_coconut_tco  #101 (line num in coconut source)
def count_vowels(word):  #101 (line num in coconut source)
    return _coconut_tail_call((len), (tuple)((filter)(lambda x: x in 'aeiouy', word)))  #101 (line num in coconut source)

def reveal_4_vowel_words(word):  #103 (line num in coconut source)
    if ((count_vowels)(word)) == 4:  #104 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #105 (line num in coconut source)
            print(word)  #106 (line num in coconut source)

def reveal_5_vowel_words(word):  #108 (line num in coconut source)
    if ((count_vowels)(word)) == 5:  #109 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #110 (line num in coconut source)
            print(word)  #111 (line num in coconut source)
