...
import words as wordle_words  #2 (line num in coconut source)
from lib_rbon import flip  #3 (line num in coconut source)
from lib_rbon import uncurry  #3 (line num in coconut source)
from lib_rbon import split  #3 (line num in coconut source)
import time  #4 (line num in coconut source)

class Info(_coconut.collections.namedtuple("Info", ('greens', 'yellows', 'bads', 'unknown', 'words')), _coconut.object):  #6 (line num in coconut source)
    __slots__ = ()  #6 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #6 (line num in coconut source)
    def __eq__(self, other):  #6 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #6 (line num in coconut source)
    def __hash__(self):  #6 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #6 (line num in coconut source)
    __match_args__ = ('greens', 'yellows', 'bads', 'unknown', 'words')  #6 (line num in coconut source)
    def __new__(_coconut_cls, greens=[], yellows=[], bads=[], unknown=[], words=()):  #6 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (greens, yellows, bads, unknown, words))  #6 (line num in coconut source)


_coconut_call_set_names(Info)  #8 (line num in coconut source)
class State(_coconut.collections.namedtuple("State", ('words', 'greens', 'yellows', 'bads', 'unknown', 'board')), _coconut.object):  #8 (line num in coconut source)
    __slots__ = ()  #8 (line num in coconut source)
    __ne__ = _coconut.object.__ne__  #8 (line num in coconut source)
    def __eq__(self, other):  #8 (line num in coconut source)
        return self.__class__ is other.__class__ and _coconut.tuple.__eq__(self, other)  #8 (line num in coconut source)
    def __hash__(self):  #8 (line num in coconut source)
        return _coconut.tuple.__hash__(self) ^ hash(self.__class__)  #8 (line num in coconut source)
    __match_args__ = ('words', 'greens', 'yellows', 'bads', 'unknown', 'board')  #8 (line num in coconut source)
    def __new__(_coconut_cls, words=(), greens=[], yellows=[], bads=[], unknown=[], board=[]):  #8 (line num in coconut source)
        return _coconut.tuple.__new__(_coconut_cls, (words, greens, yellows, bads, unknown, board))  #8 (line num in coconut source)



_coconut_call_set_names(State)  #11 (line num in coconut source)
@_coconut_tco  #11 (line num in coconut source)
def parse(command):  #11 (line num in coconut source)
    pairs = (zip)(*(split)(" = ", command))  #12 (line num in coconut source)
    greens = []  #13 (line num in coconut source)
    yellows = []  #14 (line num in coconut source)
    bads = []  #15 (line num in coconut source)
    unknown = [0, 1, 2, 3, 4]  #16 (line num in coconut source)
    index = 0  #17 (line num in coconut source)
    for i in pairs:  #18 (line num in coconut source)
        if i[1] == 'g':  #19 (line num in coconut source)
            greens.append((i[0], index))  #20 (line num in coconut source)
            if index in unknown:  #21 (line num in coconut source)
                unknown.remove(index)  #22 (line num in coconut source)
        elif i[1] == 'y':  #23 (line num in coconut source)
            yellows.append((i[0], index))  #24 (line num in coconut source)
        elif i[1] == 'b':  #25 (line num in coconut source)
            bads.append((i[0]))  #26 (line num in coconut source)
        index += 1  #27 (line num in coconut source)
    return _coconut_tail_call(Info, greens, yellows, bads, unknown)  #28 (line num in coconut source)

@_coconut_tco  #30 (line num in coconut source)
def info_add(info1, info2):  #30 (line num in coconut source)
    greens = info1.greens + info2.greens  #31 (line num in coconut source)
    yellows = info1.yellows + info2.yellows  #32 (line num in coconut source)
    bads = info1.bads + info2.bads  #33 (line num in coconut source)
    unknown = info1.unknown + info2.unknown  #34 (line num in coconut source)
    return _coconut_tail_call(Info, greens, yellows, bads, unknown)  #35 (line num in coconut source)

def step(command):  #37 (line num in coconut source)

    info = (parse)(command)  #39 (line num in coconut source)

    greens = info.greens  #41 (line num in coconut source)
    yellows = info.yellows  #42 (line num in coconut source)
    bads = info.bads  #43 (line num in coconut source)
    unknown = info.unknown  #44 (line num in coconut source)

    (print)("filtering words...")  #46 (line num in coconut source)
    start_time = time.perf_counter()  #47 (line num in coconut source)
# possible_words = possible_words \
    possible_words = (list)((filter)(_coconut.functools.partial(check_word, greens, yellows, unknown), (fmap)(tuple, wordle_words.all_words)))  # this is also needed for test parity  #49 (line num in coconut source)
    end_time = time.perf_counter()  #53 (line num in coconut source)
    (print)("finished in: " + ((str)(end_time - start_time)))  #54 (line num in coconut source)
    print("total words: " + str(len(possible_words)) + '\n')  #55 (line num in coconut source)

    return (possible_words)  #57 (line num in coconut source)

@_coconut_tco  #59 (line num in coconut source)
def new_parse(command):  #59 (line num in coconut source)
    pairs = (zip)(*(split)(" = ", command))  #60 (line num in coconut source)
    greens = []  #61 (line num in coconut source)
    yellows = []  #62 (line num in coconut source)
    bads = []  #63 (line num in coconut source)
    unknown = [0, 1, 2, 3, 4]  #64 (line num in coconut source)
    index = 0  #65 (line num in coconut source)
    for i in pairs:  #66 (line num in coconut source)
        if i[1] == 'g':  #67 (line num in coconut source)
            greens.append((i[0], index))  #68 (line num in coconut source)
            if index in unknown:  #69 (line num in coconut source)
                unknown.remove(index)  #70 (line num in coconut source)
        elif i[1] == 'y':  #71 (line num in coconut source)
            yellows.append((i[0], index))  #72 (line num in coconut source)
        elif i[1] == 'b':  #73 (line num in coconut source)
            bads.append((i[0]))  #74 (line num in coconut source)
        index += 1  #75 (line num in coconut source)
    return _coconut_tail_call(State, (), greens, yellows, bads, unknown, [(tuple)(pairs), ])  #76 (line num in coconut source)

@_coconut_tco  #78 (line num in coconut source)
def add_state(state1, state2):  #78 (line num in coconut source)
    words = state1.words + state2.words  #79 (line num in coconut source)
    greens = state1.greens + state2.greens  #80 (line num in coconut source)
    yellows = state1.yellows + state2.yellows  #81 (line num in coconut source)
    bads = state1.bads + state2.bads  #82 (line num in coconut source)
    unknown = state1.unknown + state2.unknown  #83 (line num in coconut source)
    board = state1.board + state2.board  #84 (line num in coconut source)
    return _coconut_tail_call(State, words, greens, yellows, bads, unknown, board)  #85 (line num in coconut source)

@_coconut_tco  #87 (line num in coconut source)
def new_step(command, state=State(), cheat_level=1):  #87 (line num in coconut source)

    new_state = (new_parse)(command)  #89 (line num in coconut source)
    state = add_state(state, new_state)  #90 (line num in coconut source)

    if len(state.words) == 0:  #92 (line num in coconut source)
        _coconut_case_match_to_0 = cheat_level  #93 (line num in coconut source)
        _coconut_case_match_check_0 = False  #93 (line num in coconut source)
        if _coconut_case_match_to_0 == 0:  #93 (line num in coconut source)
            _coconut_case_match_check_0 = True  #93 (line num in coconut source)
        if _coconut_case_match_check_0:  #93 (line num in coconut source)
            (print)("generating words sans bad letters...")  #95 (line num in coconut source)
            start_time = time.perf_counter()  #96 (line num in coconut source)
            words = generate_words(state.bads)  #97 (line num in coconut source)
            end_time = time.perf_counter()  #98 (line num in coconut source)
            (print)("finished in: " + ((str)(end_time - start_time)))  #99 (line num in coconut source)
            (print)('total words: ' + ((str)((len)(words))) + '\n')  #100 (line num in coconut source)
        if not _coconut_case_match_check_0:  #101 (line num in coconut source)
            if _coconut_case_match_to_0 == 1:  #101 (line num in coconut source)
                _coconut_case_match_check_0 = True  #101 (line num in coconut source)
            if _coconut_case_match_check_0:  #101 (line num in coconut source)
                words = wordle_words.all_words  #102 (line num in coconut source)
        if not _coconut_case_match_check_0:  #103 (line num in coconut source)
            if _coconut_case_match_to_0 == 2:  #103 (line num in coconut source)
                _coconut_case_match_check_0 = True  #103 (line num in coconut source)
            if _coconut_case_match_check_0:  #103 (line num in coconut source)
                words = wordle_words.possible_solutions  #104 (line num in coconut source)
    else:  #105 (line num in coconut source)
        words = state.words  #106 (line num in coconut source)

    (print)("filtering words...")  #108 (line num in coconut source)
    start_time = time.perf_counter()  #109 (line num in coconut source)
    words = filter_words(words, state.greens, state.yellows, state.unknown, state.bads)  #110 (line num in coconut source)
    end_time = time.perf_counter()  #111 (line num in coconut source)
    (print)("finished in: " + ((str)(end_time - start_time)))  #112 (line num in coconut source)
    print("total words: " + str(len(words)) + '\n')  #113 (line num in coconut source)

    return _coconut_tail_call(State, words, state.greens, state.yellows, state.bads, state.unknown, state.board)  #115 (line num in coconut source)

def generate_words(bads):  #123 (line num in coconut source)
    alphabet = 'abcdefghijklmnopqrstuvwxyz'  #124 (line num in coconut source)
    chars = (tuple)((filter)(lambda y: y not in bads, alphabet))  #125 (line num in coconut source)
    possible_words = (tuple)(product(chars, repeat=5))  #126 (line num in coconut source)
    return (possible_words)  #127 (line num in coconut source)

@_coconut_tco  #129 (line num in coconut source)
def filter_words(words, greens, yellows, unknown, bads):  #129 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(_coconut.functools.partial(check_word, greens, yellows, unknown, bads), (fmap)(tuple, words)))  #130 (line num in coconut source)

def fits_green(word, green):  #135 (line num in coconut source)
    return (word[green[1]] == green[0])  #135 (line num in coconut source)

def fits_all_greens(greens, word):  #137 (line num in coconut source)
    for green in greens:  #138 (line num in coconut source)
        if word[green[1]] != green[0]:  #139 (line num in coconut source)
            return (False)  #140 (line num in coconut source)
    return (True)  #141 (line num in coconut source)

def fits_yellow(word, unknown, yellow):  #143 (line num in coconut source)
    if word[yellow[1]] != yellow[0]:  #144 (line num in coconut source)
        res = False  #145 (line num in coconut source)
        for u in unknown:  #146 (line num in coconut source)
            if word[u] == yellow[0]:  #147 (line num in coconut source)
                return (True)  #148 (line num in coconut source)
    return (False)  #149 (line num in coconut source)

def fits_all_yellows(yellows, unknown, word):  #151 (line num in coconut source)
    for yellow in yellows:  #152 (line num in coconut source)
        if not fits_yellow(word, unknown, yellow):  #153 (line num in coconut source)
            return (False)  #154 (line num in coconut source)
    return (True)  #155 (line num in coconut source)

def bad_letters_in_word(bads, word):  #157 (line num in coconut source)
    for bad in bads:  #158 (line num in coconut source)
        if bad in word:  #159 (line num in coconut source)
            return (True)  #160 (line num in coconut source)
    return (False)  #161 (line num in coconut source)

def check_word(greens, yellows, unknown, bads, word):  #163 (line num in coconut source)
    if not (bad_letters_in_word)(bads, word):  #164 (line num in coconut source)
        if (fits_all_greens)(greens, word):  #165 (line num in coconut source)
            if (fits_all_yellows)(yellows, unknown, word):  #166 (line num in coconut source)
# if ''.join(word) in words.all_words:
                return (True)  #168 (line num in coconut source)
    return (False)  #169 (line num in coconut source)

@_coconut_tco  #171 (line num in coconut source)
def count_vowels(word):  #171 (line num in coconut source)
    return _coconut_tail_call((len), (tuple)((filter)(lambda x: x in 'aeiouy', word)))  #171 (line num in coconut source)

def reveal_4_vowel_words(word):  #173 (line num in coconut source)
    if ((count_vowels)(word)) == 4:  #174 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #175 (line num in coconut source)
            print(word)  #176 (line num in coconut source)

def reveal_5_vowel_words(word):  #178 (line num in coconut source)
    if ((count_vowels)(word)) == 5:  #179 (line num in coconut source)
        if not (len(word) != len(set(word))):  # filter dupes  #180 (line num in coconut source)
            print(word)  #181 (line num in coconut source)

@_coconut_tco  #183 (line num in coconut source)
def fancy_format(iterable):  #183 (line num in coconut source)
    return _coconut_tail_call('\n'.join, tuple(map(''.join, iterable)))  #183 (line num in coconut source)

def fancy_print(state, word_max=1000):  #185 (line num in coconut source)
    words = (fancy_format)(state.words)  #186 (line num in coconut source)
    if (len)(state.words) > word_max:  #187 (line num in coconut source)
        print("Too many words to print.")  #188 (line num in coconut source)
    else:  #189 (line num in coconut source)
        print(words)  #190 (line num in coconut source)
    print("Total words: " + str(len(state.words)))  #191 (line num in coconut source)
    print("Greens: " + ((str)(state.greens)))  #192 (line num in coconut source)
    print("Yellows: " + ((str)(state.yellows)))  #193 (line num in coconut source)
    print("Bads: " + ((str)(state.bads)))  #194 (line num in coconut source)
    print()  #195 (line num in coconut source)
    print("Board: \n" + color_board(state.board))  #196 (line num in coconut source)

@_coconut_tco  #198 (line num in coconut source)
def color_board(board):  #198 (line num in coconut source)
    return _coconut_tail_call(('\n'.join), (fmap)(color_guess, board))  #198 (line num in coconut source)

@_coconut_tco  #200 (line num in coconut source)
def color_guess(guess):  #200 (line num in coconut source)
    return _coconut_tail_call((''.join), (fmap)(color_letter, guess))  #200 (line num in coconut source)

@_coconut_tco  #202 (line num in coconut source)
def color_letter(letter):  #202 (line num in coconut source)
    _coconut_case_match_to_1 = letter[1]  #203 (line num in coconut source)
    _coconut_case_match_check_1 = False  #203 (line num in coconut source)
    if _coconut_case_match_to_1 == 'g':  #203 (line num in coconut source)
        _coconut_case_match_check_1 = True  #203 (line num in coconut source)
    if _coconut_case_match_check_1:  #203 (line num in coconut source)
        return _coconut_tail_call("{_coconut_format_0}{_coconut_format_1}{_coconut_format_2}".format, _coconut_format_0=(bcolors.GREEN), _coconut_format_1=(letter[0]), _coconut_format_2=(bcolors.ENDC))  #205 (line num in coconut source)
    if not _coconut_case_match_check_1:  #206 (line num in coconut source)
        if _coconut_case_match_to_1 == 'y':  #206 (line num in coconut source)
            _coconut_case_match_check_1 = True  #206 (line num in coconut source)
        if _coconut_case_match_check_1:  #206 (line num in coconut source)
            return _coconut_tail_call("{_coconut_format_0}{_coconut_format_1}{_coconut_format_2}".format, _coconut_format_0=(bcolors.YELLOW), _coconut_format_1=(letter[0]), _coconut_format_2=(bcolors.ENDC))  #207 (line num in coconut source)
    if not _coconut_case_match_check_1:  #208 (line num in coconut source)
        if _coconut_case_match_to_1 == 'b':  #208 (line num in coconut source)
            _coconut_case_match_check_1 = True  #208 (line num in coconut source)
        if _coconut_case_match_check_1:  #208 (line num in coconut source)
            return _coconut_tail_call("{_coconut_format_0}{_coconut_format_1}{_coconut_format_2}".format, _coconut_format_0=(bcolors.BLACK), _coconut_format_1=(letter[0]), _coconut_format_2=(bcolors.ENDC))  #209 (line num in coconut source)

def main():  #211 (line num in coconut source)
    state = State()  #212 (line num in coconut source)
# thing.strip() |> print
    while True:  #214 (line num in coconut source)
        print()  #215 (line num in coconut source)
...
# print_black("Warning: No active frommets remain. Continue?")
        i = input("yes?: ")  #219 (line num in coconut source)
        state = (new_step)(*(i, state), cheat_level=1)  #220 (line num in coconut source)
        (fancy_print)(state)  #221 (line num in coconut source)

@_coconut_tco  #223 (line num in coconut source)
def color_green(text):  #223 (line num in coconut source)
    return _coconut_tail_call("{_coconut_format_0}{_coconut_format_1}{_coconut_format_2}".format, _coconut_format_0=(bcolors.GREEN), _coconut_format_1=(text), _coconut_format_2=(bcolors.ENDC))  #223 (line num in coconut source)

@_coconut_tco  #225 (line num in coconut source)
def color_yellow(text):  #225 (line num in coconut source)
    return _coconut_tail_call("{_coconut_format_0}{_coconut_format_1}{_coconut_format_2}".format, _coconut_format_0=(bcolors.YELLOW), _coconut_format_1=(text), _coconut_format_2=(bcolors.ENDC))  #225 (line num in coconut source)

@_coconut_tco  #227 (line num in coconut source)
def color_black(text):  #227 (line num in coconut source)
    return _coconut_tail_call("{_coconut_format_0}{_coconut_format_1}{_coconut_format_2}".format, _coconut_format_0=(bcolors.BLACK), _coconut_format_1=(text), _coconut_format_2=(bcolors.ENDC))  #227 (line num in coconut source)

class bcolors(_coconut.object):  #229 (line num in coconut source)
    GREEN = '\033[97;42m'  #230 (line num in coconut source)
    YELLOW = '\033[97;43m'  #231 (line num in coconut source)
    BLACK = '\033[97;100m'  #232 (line num in coconut source)
    ENDC = '\033[0m'  #233 (line num in coconut source)

_coconut_call_set_names(bcolors)  #235 (line num in coconut source)
if __name__ == '__main__':  #235 (line num in coconut source)
# words.all_words |> fmap $ reveal_4_vowel_words
# "audio = ybgby" |> step #|> fancy_format \
# ("apdlp = ggbgy", State()) |*> new_step |> print #|> fancy_format \
# |> print
    main()  #240 (line num in coconut source)
