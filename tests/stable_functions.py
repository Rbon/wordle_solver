...
import words  #2 (line num in coconut source)
...
def fancy_print(iterable):  #6 (line num in coconut source)
    return _coconut_tail_call('\n'.join, tuple(map(''.join, iterable)))  #6 (line num in coconut source)

@_coconut_tco  #8 (line num in coconut source)
def filter_at_position(xs, y, pos):  #8 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda x: x[pos] == y, xs))  #8 (line num in coconut source)

@_coconut_tco  #10 (line num in coconut source)
def filter_not_at_position(xs, y, pos):  #10 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda x: x[pos] != y, xs))  #11 (line num in coconut source)

@_coconut_tco  #15 (line num in coconut source)
def filter_contains(xs, y):  #15 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda x: y in x, xs))  #15 (line num in coconut source)

def generate_words(chars):  #17 (line num in coconut source)
    (print)("generating words sans bad letters...")  #18 (line num in coconut source)
    output = (tuple)(product(chars, repeat=5))  #19 (line num in coconut source)
    (print)('total words: ' + ((str)((len)(output))))  #20 (line num in coconut source)
    return (output)  #21 (line num in coconut source)

def make_info(pairs):  #23 (line num in coconut source)
    greens = []  #24 (line num in coconut source)
    yellows = []  #25 (line num in coconut source)
    bads = []  #26 (line num in coconut source)
    unknown = [0, 1, 2, 3, 4]  #27 (line num in coconut source)
    index = 0  #28 (line num in coconut source)
    for i in pairs:  #29 (line num in coconut source)
        if i[1] == 'g':  #30 (line num in coconut source)
            greens.append((i[0], index))  #31 (line num in coconut source)
            if index in unknown:  #32 (line num in coconut source)
                unknown.remove(index)  #33 (line num in coconut source)
        elif i[1] == 'y':  #34 (line num in coconut source)
            yellows.append((i[0], index))  #35 (line num in coconut source)
        elif i[1] == 'b':  #36 (line num in coconut source)
            bads.append((i[0]))  #37 (line num in coconut source)
        index += 1  #38 (line num in coconut source)
    return ({'greens': greens, 'yellows': yellows, 'bads': bads, 'unknown': unknown})  #39 (line num in coconut source)

@_coconut_tco  #46 (line num in coconut source)
def parse(command):  #46 (line num in coconut source)
    return _coconut_tail_call((make_info), (zip)(*(split)(" = ", command)))  #46 (line num in coconut source)

@_coconut_tco  #48 (line num in coconut source)
def remove_matches(xs, ys):  #48 (line num in coconut source)
    return _coconut_tail_call((tuple), (filter)(lambda y: y not in xs, ys))  #48 (line num in coconut source)

@_coconut_tco  #50 (line num in coconut source)
def step(command):  #50 (line num in coconut source)
    info = (parse)(command)  #51 (line num in coconut source)
    return _coconut_tail_call((exec_time), filter_actual_words, (exec_time)(_coconut.functools.partial(filter_with_yellows, info['yellows'], info['unknown']), (exec_time)(_coconut.functools.partial(filter_with_greens, info['greens']), (exec_time)(generate_words, (remove_matches)(info["bads"], 'abcdefghijklmnopqrstuvwxyz')))))  #52 (line num in coconut source)

def exec_time(f, *args):  #59 (line num in coconut source)
    start_time = time.perf_counter()  #60 (line num in coconut source)
    output = f(*args)  #61 (line num in coconut source)
    end_time = time.perf_counter()  #62 (line num in coconut source)
    (print)("finished in: " + ((str)(end_time - start_time)) + '\n')  #63 (line num in coconut source)
    return (output)  #64 (line num in coconut source)
...
def filter_actual_words(guesses):  #101 (line num in coconut source)
    (print)("filtering actual words...")  #102 (line num in coconut source)
    output = [x for x in guesses if ''.join(x) in words.all_words]  #103 (line num in coconut source)
    (print)('possible solutions: ' + ((str)((len)((tuple)(output)))))  #104 (line num in coconut source)
    return (output)  #105 (line num in coconut source)
